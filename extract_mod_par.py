...
class mod_parameters(object):
    ...
    def ModAmptoTilt(self, month, savefig=0):
        ...
        def extract_params(day_str, path_to_data='../../wvr1_data_local/'):
            ...
            for fl in os.listdir(path_to_data):
                ...
                if fl[0:8]==day_str:
                    ...
                    #fl=path_to_data+folder+'/'+folder+'_fast.txt'
                        ...
                        params_data['p_single_err'].append(p_err_single)
                        params_data['p_double'].append(p_double)
                        params_data['p_double_err'].append(p_err_double)
                        print(day, time)
                        print('p_double, p_err_double, p_single, p_err_single:')
                        print(p_double, p_err_double, p_single, p_err_single)
            ...
            for i in range(len(day)):
                ...
                #
            params_data['date_xaxis']=date_xaxis
            print(date_xaxis)
            #print('params_data[p_single]=', params_data['p_single'])
            #
        ...
        if not os.path.exists(outpath):
            os.makedirs(outpath)
        if not os.path.exists(outhpath_singleslabfit):
            os.makedirs(outhpath_singleslabfit)
        ...
        for day_str in day_str_list_full:
            ...
            if not os.path.exists(pickle_fn):
                ...
                params_data = extract_params(day_str)
                #print(params_data)
                f = open(pickle_fn,'wb')
                pk.dump(params_data, f)
                f.close()
            else:
                print(pickle_fn+' already exists.\n')
            ...
            for i in range(len(date_xaxis)):
                ...
                if convert_to_tilt==1:
                    ...
                    if os.path.exists(pickle_deltas):
                        ...
                        z_offs, z_offs_err, delta=x_am.create_am_datfile(filelist, path_to_data=path+folderlist+'/', template='SPole_annual_50.amc', spline=2, showplots=0, write_dat=0)
    ...
    # def extract_monthly_mod(self, month, template='SPole_annual_50.amc', outpath='../mod_data/'):
    #
    #     day_str_list_full=[]
    #
    #     for d in range(1, 30):
    ...
    #         #filelist=day_str+'_'+time+'_scanAz_fast.txt'
    #         path=outpath
    #         pickle_fn=outpath+day_str+'_mod_parameters_constrained_pk.txt'
    #         f = open(pickle_fn,'wb')
    #         day=[]
    #         time=[]
    #         params_data = {'date_xaxis': [], 'p_single':[], 'p_single_err':[], 'p_double':[], 'p_double_err':[]}
    ...
    #             print(folderlist)
    #             if folderlist[0:8]==day_str:
    #                 if folderlist[-6:]=='scanAz':
    #                     path_to_test=path+folderlist+'/'
    ...
    #                             D_clean, waz, mod_removed_data, p_double, p_err_double, p_single, p_err_single = x.read_Az_fast(filelist, pathtofn=folderlist+'/', clean_method='fit')
