...
class reduc_wvr_pager(initialize):
    ...
    def makeFileListFromData(self,typ='*',start=None, end=None):
        ...
        removeFiles = ['2016wvrLog.tar.gz','2017wvrLog.tar.gz','2017_Wx_Spo_NOAA.tar.gz', 'MMCR','2018wvrLog.tar.gz','2019wvrLog.tar.gz']
        ...
        self.dt_list = [datetime.strptime(tmp[0:15],'%Y%m%d_%H%M%S') for tmp in self.fileList]
    ...
    def checkAzSkips(self, fileList=None, verb=True):
          
        nowm1hr = (datetime.now()-timedelta(minutes=60)).strftime('%Y%m%d_%H*')
        cwd = os.getcwd()
        os.chdir(self.dataDir)
        if fileList is None:
            fl=sort(glob.glob('%s*skyDip*log*'%nowm1hr))
        else:
            fl = fileList
        dA = []
        nfiles = len(fileList)
        ...
        for fileName in fl:
            fileName = fileName.replace('.tar.gz','_log.txt')
            f = open(fileName,'r')
            lines = f.readlines()
            f.close()
            dA=[]
            ...
            if size(dA) == 0:
                if verb:
                    print "AzSkipCheck: no az homing in %s" %fileName
                continue
            ...
            if size(dA) == 0:
                ...
                passfail = (dA[idx] == 0) or ((dA[idx] > 359) and (dA[idx] < 361) or (dA[idx] == -39600))
            ...
            print "AzSkipCheck: deltaAz: %.2f deg, File: %s" % (dA[idx], fileName)
            self.passfailmsg("AzSkipCheck", passfail)

        print "Nfiles: %d, NFail:%d, Npass:%d"%(nfiles, nfiles-npass, npass)
    ...
    def checkFileSizeStatus(self, time=0, prefix='log',thres = 1e4, verb=True):
        ...
        cwd= os.getcwd()
        os.chdir(self.dataDir)
        cmd = 'ls -lrt $(d ate +'+fmt+ ' -d\"%d days ago\")*%s*'%(time,prefix)
        p = Popen(cmd, stdout=PIPE, stderr=PIPE,shell=True)
        aout,aerr = p.communicate()
        os.chdir(cwd)
